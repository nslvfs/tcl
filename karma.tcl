...
# some minor changes by vfs 03/2015
...
proc fixkarma {nick uhost hand chan text} {
  ...
  if {[string match "*+lkarma*" [channel info $chan]]} {  
    ...
    if { [regexp -nocase {(.*)(\+|\-)$} $item] } {
      puthelp "PRIVMSG $chan :\001ACTION sets mode -hirn $nick\001"
      return 0
    }
    if {$item == ""} {
      puthelp "PRIVMSG $chan :Was gedenkst du zu ändern?"
      return 0
    }
    if {[string match -nocase *$nick* $item]} {
      puthelp "PRIVMSG $chan :Das eigene Karma zu ändern ist eine Sünde - Frag peiou"
      return 0
    }
    if {[kdb eval {SELECT locked FROM lkarma WHERE item=$item}] == "Y"} { 
      set lockedk [kdb eval {SELECT karma FROM lkarma WHERE item=$item}]
      if {$lockedk == 0} {
        puthelp "PRIVMSG $chan :Du kannst das Karma von \002$item\002 nicht beeinflussen!"
      } {   
        puthelp "PRIVMSG $chan :Das Karma von \002$item\002 ist gesperrt auf \002$lockedk\002."
      }
  	  return 0
    }
    ...
    if {$kfcount($uhost:$chan) == [lindex $kfflood 0]} {
      puthelp "PRIVMSG $chan :HALT DIE FRESSE $nick!"
      return 0
    }
    ...
    puthelp "PRIVMSG $chan :Karma für \002$item\002 ist jetzt \002$newkarma\002."
    kdb close 
    karmaupdate 
  }
}
